...
- **여러 기기에서 AI 도구를 사용하나요?** 모든 토큰 사용량을 한 곳에서 확인 — PromQL로 쿼리하고, 웹 대시보드나 [Toki Monitor](https://github.com/korjwl1/toki-monitor)에서 시각화할 수 있습니다.

- **팀 사용량 대시보드가 필요하나요?** 팀 범위 PromQL 쿼리와 역할 기반 접근 제어로 팀원들의 토큰 사용량을 집계합니다.
...
- **toki-sync 서버**가 사용자를 인증하고, 메타데이터를 SQLite/PostgreSQL에 저장하며, 시계열을 VictoriaMetrics에 기록
- **PromQL 프록시**가 사용자별 label을 주입하여 데이터 격리 — 각 사용자는 자신의 데이터만 조회 가능
...
- **사용자별 데이터 격리** — PromQL 프록시가 사용자 label을 주입하여 각 사용자는 자신의 데이터만 조회할 수 있습니다.
