...
impl PostgresRepo {
    ...
    async fn migrate(&self) -> Result<()> {
        ...
        if user_count == 0 {
            ...
            sqlx::query(
                "INSERT INTO users (id, username, password_hash, role, created_at, updated_at, active) VALUES ($1, 'admin', $2, 'admin', $3, $4, 1)"
            )
            .bind(&id)
            .bind(&hash)
            .bind(now)
...
impl DatabaseRepo for PostgresRepo {
    ...
    async fn get_user_by_username(&self, username: &str) -> Result<Option<User>> {
        let row: Option<(String, String, String, String, i64, i64, Option<String>, Option<String>, bool)> = sqlx::query_as(
            "SELECT id, username, password_hash, role, created_at, updated_at, oidc_sub, oidc_issuer, active != 0 FROM users WHERE username = $1",
        )
        .bind(username)
    ...
    async fn get_user_by_id(&self, id: &str) -> Result<Option<User>> {
        let row: Option<(String, String, String, String, i64, i64, Option<String>, Option<String>, bool)> = sqlx::query_as(
            "SELECT id, username, password_hash, role, created_at, updated_at, oidc_sub, oidc_issuer, active != 0 FROM users WHERE id = $1",
        )
        .bind(id)
        .fetch_optional(&self.pool)
        .await
    ...
    async fn list_user_devices(&self, user_id: &str) -> Result<Vec<DeviceSummary>> {
        let rows: Vec<(String, String, String, i64)> = sqlx::query_as(
            "SELECT id, name, COALESCE(device_key, '') AS device_key, last_seen_at FROM devices WHERE user_id = $1 ORDER BY last_seen_at DESC",
        )
        .bind(user_id)
        .fetch_all(&self.pool)
        .await
    ...
    async fn update_user_role(&self, user_id: &str, role: &str) -> Result<bool> {
        let now = chrono::Utc::now().timestamp();
        let affected = sqlx::query("UPDATE users SET role = $1, updated_at = $2 WHERE id = $3")
            .bind(role)
            .bind(now)
            .bind(user_id)
            .execute(&self.pool)
    ...
    async fn approve_registration(&self, id: &str) -> Result<bool> {
        ...
        let row: Option<(String, String, String, i64)> = sqlx::query_as(
            "SELECT id, username, password_hash, requested_at FROM pending_registrations WHERE id = $1",
        )
        .bind(id)
        ...
        let exists: bool = sqlx::query_scalar("SELECT COUNT(*) > 0 FROM users WHERE username = $1")
            .bind(&username)
            .fetch_one(&mut *tx)
            .await?;
        ...
        sqlx::query(
            "INSERT INTO users (id, username, password_hash, role, created_at, updated_at) VALUES ($1, $2, $3, 'user', $4, $5)",
        )
        .bind(&user_id)
        .bind(&username)
        .bind(&password_hash)
    ...
    async fn reject_registration(&self, id: &str) -> Result<bool> {
        let affected = sqlx::query("DELETE FROM pending_registrations WHERE id = $1")
            .bind(id)
            .execute(&self.pool)
            .await
    ...
    async fn cleanup_old_pending_registrations(&self, max_age_secs: i64) -> Result<u64> {
        ...
        let result = sqlx::query("DELETE FROM pending_registrations WHERE requested_at < $1")
            .bind(cutoff)
            .execute(&self.pool)
            .await
    ...
    async fn find_user_by_oidc(&self, issuer: &str, sub: &str) -> Result<Option<User>> {
        let row: Option<(String, String, String, String, i64, i64, Option<String>, Option<String>, bool)> = sqlx::query_as(
            "SELECT id, username, password_hash, role, created_at, updated_at, oidc_sub, oidc_issuer, active != 0 FROM users WHERE oidc_issuer = $1 AND oidc_sub = $2",
        )
        .bind(issuer)
        .bind(sub)
        .fetch_optional(&self.pool)
    ...
    async fn set_user_active(&self, user_id: &str, active: bool) -> Result<bool> {
        ...
        let affected = sqlx::query("UPDATE users SET active = $1, updated_at = $2 WHERE id = $3")
            .bind(val)
            .bind(now)
            .bind(user_id)
            .execute(&self.pool)
    ...
    async fn list_monitor_settings(&self, user_id: &str) -> Result<Vec<MonitorSetting>> {
        let rows: Vec<(String, String, i64, i64)> = sqlx::query_as(
            "SELECT key, value, version, updated_at FROM monitor_settings WHERE user_id = $1 ORDER BY key",
        )
        .bind(user_id)
        .fetch_all(&self.pool)
        .await
    ...
    async fn list_monitor_setting_index(&self, user_id: &str) -> Result<Vec<MonitorSettingMeta>> {
        let rows: Vec<(String, i64, i64, i64)> = sqlx::query_as(
            "SELECT key, version, updated_at, size_bytes FROM monitor_settings WHERE user_id = $1 ORDER BY key",
        )
        .bind(user_id)
        .fetch_all(&self.pool)
        .await
    ...
    async fn monitor_settings_usage(&self, user_id: &str) -> Result<MonitorUsage> {
        let (entries, total_bytes): (i64, i64) = sqlx::query_as(
            "SELECT COUNT(*)::BIGINT, COALESCE(SUM(size_bytes), 0)::BIGINT
             FROM monitor_settings WHERE user_id = $1",
        )
        .bind(user_id)
        .fetch_one(&self.pool)
        .await
    ...
    async fn count_active_admins_except(&self, username: &str) -> Result<i64> {
        let count: i64 = sqlx::query_scalar(
            "SELECT COUNT(*)::BIGINT FROM users WHERE role = 'admin' AND active = 1 AND username != $1"
        )
        .bind(username)
        .fetch_one(&self.pool)
        .await
