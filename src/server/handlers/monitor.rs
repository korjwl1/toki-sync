...
mod tests {
    ...
    impl Harness {
        async fn new(users: &[&str]) -> Self {
            ...
            let db = Arc::new(
                SqliteRepo::open(db_file.path().to_str().unwrap())
                    .await
                    .unwrap(),
            );
            for u in users {
            ...
            let state = crate::server::http::AppState {
                ...
                trust_proxy: false,
                pricing: Arc::new(tokio::sync::RwLock::new(crate::pricing::PricingTable::new(
                    HashMap::new(),
                ))),
    ...
    async fn entries_are_isolated_per_user() {
        ...
        h.put(
            "/me/monitor/settings/dash:main",
            "alice",
            serde_json::json!({ "value": "alice-payload" }),
        )
        .await;
        h.put(
            "/me/monitor/settings/dash:main",
            "bob",
            serde_json::json!({ "value": "bob-payload" }),
        )
        .await;
        h.put(
        ...
        assert_eq!(
            keys,
            vec!["dash:main"],
            "bob's index must not name alice's keys"
        );
        ...
        assert_eq!(
            s,
            StatusCode::CONFLICT,
            "bob must not CAS onto a key he does not have"
        );
        ...
        assert!(h
            .db
            .get_monitor_setting("alice", "alice.only")
            .await
            .unwrap()
            .is_some());
    }
    ...
    async fn a_blind_writer_learns_it_clobbered_someone() {
        ...
        let (_, b) = h
            .put(
                "/me/monitor/settings/k",
                "alice",
                serde_json::json!({ "value": "from-b" }),
            )
            .await;
        assert_eq!(b["previous_version"], 2);
        assert_ne!(
            b["previous_version"], fetched_version,
            "B must be able to see it clobbered A"
        );
    ...
    async fn an_oversized_value_is_refused_with_the_limit_named() {
        ...
        assert!(v["error"]
            .as_str()
            .unwrap()
            .contains(&MAX_VALUE_BYTES.to_string()));
    ...
    async fn per_user_quota_counts_entries_and_bytes() {
        ...
        for key in ["a", "b"] {
            let out =
                h.db.upsert_monitor_setting("alice", key, "1234567890", None, q)
                    .await
                    .unwrap();
        ...
        match h
            .db
            .upsert_monitor_setting("alice", "c", "x", None, q)
            .await
            .unwrap()
        {
            MonitorWriteOutcome::QuotaExceeded { what, used, limit } => {
        ...
        match h
            .db
            .upsert_monitor_setting("alice", "a", &"y".repeat(95), None, q)
            .await
            .unwrap()
        {
            MonitorWriteOutcome::QuotaExceeded { what, used, limit } => {
        ...
        let out =
            h.db.upsert_monitor_setting("alice", "a", "z", None, q)
                .await
                .unwrap();
        assert!(matches!(out, MonitorWriteOutcome::Written { .. }));
        ...
        let out =
            h.db.upsert_monitor_setting("bob", "a", "1234567890", None, q)
                .await
                .unwrap();
        assert!(matches!(out, MonitorWriteOutcome::Written { .. }));
    }
    ...
    async fn keys_outside_the_allowed_shape_are_refused() {
        ...
        for key in [
            ...
            let (s, _) = h
                .put(&uri, "alice", serde_json::json!({ "value": "v" }))
                .await;
            assert_eq!(
                s,
        ...
        for key in [
            ...
            let (s, _) = h
                .put(
                    &format!("/me/monitor/settings/{key}"),
                    "alice",
                    serde_json::json!({ "value": "v" }),
                )
                .await;
    ...
    async fn deletes_and_account_removal_leave_nothing_behind() {
        ...
        assert!(
            h.db.list_monitor_settings("alice")
                .await
                .unwrap()
                .is_empty(),
    ...
    fn the_write_limiter_allows_a_burst_and_stops_a_loop() {
        ...
        for i in 0..MonitorWriteRateLimiterInner::MAX_WRITES {
            assert!(
                limiter.allow("alice").is_ok(),
                "write {i} of a burst was refused"
            );
        }
    ...
    async fn writes_past_the_budget_answer_429_and_change_nothing() {
        ...
        for i in 0..MonitorWriteRateLimiterInner::MAX_WRITES {
            let (s, _) = h
                .put(
                    &format!("/me/monitor/settings/k{i}"),
                    "alice",
                    serde_json::json!({ "value": "v" }),
                )
                .await;
        ...
        assert!(h
            .db
            .get_monitor_setting("alice", "over")
            .await
            .unwrap()
            .is_none());
    ...
    async fn the_index_reports_size_without_carrying_the_payload() {
        ...
        assert!(
            entry["value"].is_null(),
            "the index must not carry payloads"
        );
