...
pub async fn me_rename_device(
    ...
    if name.is_empty() || name.len() > 64 {
        return Err(AppError {
            ...
            message: "device name must be 1-64 characters".into(),
        });
    }
    if name.contains(|c: char| c.is_control()) {
        return Err(AppError {
            status: StatusCode::UNPROCESSABLE_ENTITY,
            message: "device name must not contain control characters".into(),
        });
    }

    let renamed = state
        .db
        .rename_device(&device_id, &claims.sub, &name)
        .await
        .map_err(AppError::internal)?;
    if !renamed {
...
pub async fn me_delete_device(
    ...
    let belongs = state
        .db
        .device_belongs_to_user(&device_id, &claims.sub)
        .await
        .map_err(AppError::internal)?;
    if !belongs {
        return Err(AppError::not_found("device not found"));
    ...
    state
        .db
        .delete_user_device(&device_id, &claims.sub)
        .await
        .map_err(AppError::internal)?;
...
pub async fn me_change_password(
    ...
    let new_hash = tokio::task::spawn_blocking(move || bcrypt::hash(&new_pw, bcrypt::DEFAULT_COST))
        .await
        .map_err(AppError::internal)?
        .map_err(AppError::internal)?;
    ...
    state
        .db
        .revoke_user_refresh_tokens(&claims.sub)
        .await
        .map_err(AppError::internal)?;
